...
int UartPsIntrExample_RS485(INTC *IntcInstPtr, XUartPs *UartInstPtr,
			u16 DeviceId, u16 UartIntrId);


static int SetupInterruptSystem_RS485(INTC *IntcInstancePtr,
...
XUartPs UartPs1;		/* Instance of the UART Device */
INTC InterruptController1;	/* Instance of the Interrupt Controller */
...
void bild_send_buffer_RS485(u32 address, u32 data){
	...
	SendBuffer[address] = data - high_bits*256;
...
void terminal_uart_send_RS485() {
	...
	SendBuffer[2] = 3;
	SendBuffer[3] = 4;
	XUartPs_Send(&UartPs1, SendBuffer, TEST_BUFFER_SIZE);

	     while(UartPs1.SendBuffer.RemainingBytes!=0)
	     {
	    	 XUartPs_Send(&UartPs1, UartPs1.SendBuffer.NextBytePtr, UartPs1.SendBuffer.RemainingBytes);
	     }
}

u32 terminal_uart_recv_RS485() {
	return XUartPs_Recv(&UartPs1, RecvBuffer, TEST_BUFFER_SIZE);
}

void initialization_of_UART_RS485(){
	UartPsIntrExample_RS485(&InterruptController1, &UartPs1,
