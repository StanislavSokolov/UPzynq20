...
//    }
     ...
     * prompting the user for updates to the brightness level. */
    		...
    		Xil_Out32(XPAR_IP_AXI_INVERTER_0_S00_AXI_BASEADDR + 1*coef, 0x00000000);			// –азрешение работы тормозных резисторов
    	...
    	//	Xil_Out32(XPAR_IP_AXI_INVERTER_0_S00_AXI_BASEADDR + 25*coef, 0x00000000);			// врем€ AckChecker_wrapper_0 (AcknowledgeTimeGroup1 - резерв) - избыточна
    		...
    		for (int i = 0; i < 16; i++) {
    			Xil_Out32(XPAR_IP_AXI_PWM_0_S00_AXI_BASEADDR + i*coef, 0x000FFFFF+i*0x00100000);			// нулевые значени€ счетчиков дл€ IP_AXI_PWM (регистры 0-15)
    		}

    		Xil_Out32(XPAR_IP_AXI_PWM_0_S00_AXI_BASEADDR + 16*coef, 0x0000000F);				// направление пилы дл€ IP_AXI_PWM (PWMdirection)
    		Xil_Out32(XPAR_IP_AXI_PWM_0_S00_AXI_BASEADDR + 17*coef, 0x01000000);				// счетчик пилы дл€ IP_AXI_PWM (PWMcounterMax)
